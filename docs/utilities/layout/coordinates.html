...
<section class="d-stack16">
    ...
    <div class="d-stack8">
      ...
      <div class="d-w100p d-of-auto">
        <table class="d-table">
            ...
            <tbody>
                {% assign coordinates = "0, 1, 2, 4, 6, 8, 12, 16, 24, 32, 48, 64, 72, 84, 96, 102, 50%, 100%, 100-calc" | split: ", "%}
                {% for c in coordinates %}

                {% endfor %}

                {% for c in spacing.coordinates %}
                ...
                <tr>
                    ...
                    {% assign dir = i.direction %}
                    {% assign pre = i.prefix %}
                    {% assign percent = i.percent %}
                        {% if percent == "no" %}
                            {% if combo == "no" %}
                                <td class="d-o50 d-bgc-black-050 d-fc-black-400 d-fs12 d-ta-center"><em>N/A</em></td>
                            {% else %}
                                <td class="d-ff-mono d-fc-purple d-fs12">.d-{{ pre }}{{ coordinate }}{% if suffix == "-calc" %}{{ suffix }}{% endif %}</td>
                            {% endif %}
                        {% else %}
                            <td class="d-ff-mono d-fc-purple d-fs12">.d-{{ pre }}{{ coordinate }}{% if suffix == "-calc" %}{{ suffix }}{% endif %}</td>
                        {% endif %}
    ...
    <div class="d-stack8">
        <header>
            {% header "h3", "Examples", "1" %}
            {% paragraph %}Use the {% code %}top|right|bottom|left|x|y|all{% endcode %} utility classes to absolutely position elements within the nearest positioned element.{% endparagraph %}
        </header>
        <aside class="d-bar8 d-of-hidden">
            <header class="d-p24 d-bgc-purple-100 d-bgo50 d-w100p d-hmn216 d-d-grid d-g-cols4 d-gg24 d-pi-center sm:d-fs10 xl:d-fs18 d-fs24 d-fw-bold">
            ...
            <footer class="d-p8 d-bgc-black-700 d-bbr8 d-fs12 d-of-y-scroll d-hmx464">
...
<section class="d-stack16">
                        ...
                        {% assign percent = i.percent %}
                            {% if percent == "no" %}
                                {% if combo == "no" %}
                                    <td class="d-o50 d-bgc-black-050 d-fc-black-400 d-fs12 d-ta-center"><em>N/A</em></td>
                                {% else %}
                                    <td class="d-ff-mono d-fc-purple d-fs12">.d-{{ pre }}n{{ coordinate }}{% if suffix == "-calc" %}{{ suffix }}{% endif %}</td>
                                {% endif %}
                            {% else %}
                                <td class="d-ff-mono d-fc-purple d-fs12">.d-{{ pre }}n{{ coordinate }}{% if suffix == "-calc" %}{{ suffix }}{% endif %}</td>
                            {% endif %}
    ...
    <div class="d-stack8">
        <header>
            {% header "h3", "Examples", "2" %}
            {% paragraph %}Use the {% code %}top|right|bottom|left|x|y|all{% endcode %} utility classes to absolutely position elements within the nearest positioned element.{% endparagraph %}
        </header>
        <aside class="d-bar8 d-of-hidden">
            <header class="d-p24 d-bgc-yellow-100 d-bgo50 d-w100p d-hmn216 d-d-grid d-g-cols4 d-gg24 d-pi-center sm:d-fs10 xl:d-fs18 d-fs24 d-fw-bold">
