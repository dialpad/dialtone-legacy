---
title: Icon
desc: The Icon component provides a set of glyphs and sizes to provide context your application.
new: true
---

<code-well-header>
  <div class="d-d-flex d-ai-center d-gg16">
    <inbox-icon class="d-icon--size-100" />
    <inbox-icon class="d-icon--size-200" />
    <inbox-icon class="d-icon--size-300" />
    <inbox-icon class="d-icon--size-400" />
    <inbox-icon class="d-icon--size-500" />
    <inbox-icon class="d-icon--size-600" />
    <inbox-icon class="d-icon--size-700" />
    <inbox-icon class="d-icon--size-800" />
  </div>
</code-well-header>

<aside class="d-notice d-notice--info d-mt24 d-wmx100p" role="status" aria-hidden="false">
  <div class="d-notice__icon">
    <svg aria-hidden="true" focusable="false" data-name="Info" class="d-icon d-icon--info" viewBox="0 0 12 12"><g clip-path="url(#a)"><path fill="currentColor" fill-rule="evenodd" d="M6 1.5a4.5 4.5 0 1 0 0 9 4.5 4.5 0 0 0 0-9ZM.5 6a5.5 5.5 0 1 1 11 0 5.5 5.5 0 0 1-11 0Zm5-2a.5.5 0 0 1 .5-.5h.005a.5.5 0 0 1 0 1H6a.5.5 0 0 1-.5-.5ZM6 5.5a.5.5 0 0 1 .5.5v2a.5.5 0 0 1-1 0V6a.5.5 0 0 1 .5-.5Z" clip-rule="evenodd"/></g><defs><clipPath id="a"><path fill="#fff" d="M0 0h12v12H0z"/></clipPath></defs></svg>
  </div>
  <div class="d-notice__content d-stack4">
    <p class="d-notice__message">
      <strong>Looking for the previous icon set?</strong>
      <router-link class="d-link" to="/design/icons/system.html">Documentation for the deprecated icon set</router-link> will remain available until this new icon is more fully adopted.
    </p>
  </div>
</aside>

## Most common

We're in the process of updating our documentation to search all available new icons. Work with the product design team to identify any not previewed below.

<div class="d-gl-docsite-icons">
  <div class="dialtone-icon-grid__item">
    <aside class="dialtone-icon-card">
      <header class="dialtone-icon-card__header d-c-text h:d-bgc-unset">
        <div class="dialtone-icon-card__icon">
          <svg aria-hidden="true" focusable="false" data-name="Arrow Left" class="d-icon d-icon--arrowLeft" viewBox="0 0 12 12"><path fill="currentColor" fill-rule="evenodd" d="M6.354 2.146a.5.5 0 0 1 0 .708L3.707 5.5H9.5a.5.5 0 0 1 0 1H3.707l2.647 2.646a.5.5 0 1 1-.708.708l-3.5-3.5a.5.5 0 0 1 0-.708l3.5-3.5a.5.5 0 0 1 .708 0Z" clip-rule="evenodd"/></svg>
        </div>
        <p class="dialtone-icon-card__subtitle d-tt-capitalize">
          Arrow Left
        </p>
      </header>
    </aside>
  </div>
  <div class="dialtone-icon-grid__item">
    <aside class="dialtone-icon-card">
      <header class="dialtone-icon-card__header d-c-text h:d-bgc-unset">
        <div class="dialtone-icon-card__icon">
          <svg aria-hidden="true" focusable="false" data-name="Arrow Right" class="d-icon d-icon--arrowRight" viewBox="0 0 12 12"><path fill="currentColor" fill-rule="evenodd" d="M5.646 2.146a.5.5 0 0 1 .708 0l3.5 3.5a.5.5 0 0 1 0 .708l-3.5 3.5a.5.5 0 0 1-.708-.708L8.293 6.5H2.5a.5.5 0 0 1 0-1h5.793L5.646 2.854a.5.5 0 0 1 0-.708Z" clip-rule="evenodd"/></svg>
        </div>
        <p class="dialtone-icon-card__subtitle d-tt-capitalize">
          Arrow Right
        </p>
      </header>
    </aside>
  </div>
  <div class="dialtone-icon-grid__item">
    <aside class="dialtone-icon-card">
      <header class="dialtone-icon-card__header d-c-text h:d-bgc-unset">
        <div class="dialtone-icon-card__icon">
          <svg aria-hidden="true" focusable="false" data-name="Arrow Up" class="d-icon d-icon--arrowUp" viewBox="0 0 12 12"><path fill="currentColor" fill-rule="evenodd" d="M5.646 2.146a.5.5 0 0 1 .708 0l3.5 3.5a.5.5 0 0 1-.708.708L6.5 3.707V9.5a.5.5 0 0 1-1 0V3.707L2.854 6.354a.5.5 0 1 1-.708-.708l3.5-3.5Z" clip-rule="evenodd"/></svg>
        </div>
        <p class="dialtone-icon-card__subtitle d-tt-capitalize">
          Arrow Up
        </p>
      </header>
    </aside>
  </div>
  <div class="dialtone-icon-grid__item">
    <aside class="dialtone-icon-card">
      <header class="dialtone-icon-card__header d-c-text h:d-bgc-unset">
        <div class="dialtone-icon-card__icon">
          <svg aria-hidden="true" focusable="false" data-name="Arrow Down" class="d-icon d-icon--arrowDown" viewBox="0 0 12 12"><path fill="currentColor" fill-rule="evenodd" d="M6 2a.5.5 0 0 1 .5.5v5.793l2.646-2.647a.5.5 0 1 1 .708.708l-3.5 3.5a.5.5 0 0 1-.708 0l-3.5-3.5a.5.5 0 1 1 .708-.708L5.5 8.293V2.5A.5.5 0 0 1 6 2Z" clip-rule="evenodd"/></svg>
        </div>
        <p class="dialtone-icon-card__subtitle d-tt-capitalize">
          Arrow Down
        </p>
      </header>
    </aside>
  </div>
  <div class="dialtone-icon-grid__item">
    <aside class="dialtone-icon-card">
      <header class="dialtone-icon-card__header d-c-text h:d-bgc-unset">
        <div class="dialtone-icon-card__icon">
          <svg aria-hidden="true" focusable="false" data-name="Calendar" class="d-icon d-icon--calendar" viewBox="0 0 12 12"><path fill="currentColor" fill-rule="evenodd" d="M4 .5a.5.5 0 0 1 .5.5v.5h3V1a.5.5 0 0 1 1 0v.5h1A1.5 1.5 0 0 1 11 3v7a1.5 1.5 0 0 1-1.5 1.5h-7A1.5 1.5 0 0 1 1 10V3a1.5 1.5 0 0 1 1.5-1.5h1V1A.5.5 0 0 1 4 .5Zm-.5 2h-1A.5.5 0 0 0 2 3v1.5h8V3a.5.5 0 0 0-.5-.5h-1V3a.5.5 0 0 1-1 0v-.5h-3V3a.5.5 0 0 1-1 0v-.5Zm6.5 3H2V10a.5.5 0 0 0 .5.5h7a.5.5 0 0 0 .5-.5V5.5Z" clip-rule="evenodd"/></svg>
        </div>
        <p class="dialtone-icon-card__subtitle d-tt-capitalize">
          Calendar
        </p>
      </header>
    </aside>
  </div>
  <div class="dialtone-icon-grid__item">
    <aside class="dialtone-icon-card">
      <header class="dialtone-icon-card__header d-c-text h:d-bgc-unset">
        <div class="dialtone-icon-card__icon">
          <svg aria-hidden="true" focusable="false" data-name="Check" class="d-icon d-icon--check" viewBox="0 0 12 12"><path fill="currentColor" fill-rule="evenodd" d="M10.354 2.646a.5.5 0 0 1 0 .708l-5.5 5.5a.5.5 0 0 1-.708 0l-2.5-2.5a.5.5 0 1 1 .708-.708L4.5 7.793l5.146-5.147a.5.5 0 0 1 .708 0Z" clip-rule="evenodd"/></svg>
        </div>
        <p class="dialtone-icon-card__subtitle d-tt-capitalize">
          Check
        </p>
      </header>
    </aside>
  </div>
  <div class="dialtone-icon-grid__item">
    <aside class="dialtone-icon-card">
      <header class="dialtone-icon-card__header d-c-text h:d-bgc-unset">
        <div class="dialtone-icon-card__icon">
          <svg aria-hidden="true" focusable="false" data-name="Check Circle" class="d-icon d-icon--checkCircle" viewBox="0 0 12 12"><g clip-path="url(#a)"><path fill="currentColor" fill-rule="evenodd" d="M.5 6a5.5 5.5 0 1 1 11 0 5.5 5.5 0 0 1-11 0ZM6 1.5a4.5 4.5 0 1 0 0 9 4.5 4.5 0 0 0 0-9Zm1.854 3.146a.5.5 0 0 1 0 .708l-2 2a.5.5 0 0 1-.708 0l-1-1a.5.5 0 1 1 .708-.708l.646.647 1.646-1.647a.5.5 0 0 1 .708 0Z" clip-rule="evenodd"/></g><defs><clipPath id="a"><path fill="#fff" d="M0 0h12v12H0z"/></clipPath></defs></svg>
        </div>
        <p class="dialtone-icon-card__subtitle d-tt-capitalize">
          Check Circle
        </p>
      </header>
    </aside>
  </div>
  <div class="dialtone-icon-grid__item">
    <aside class="dialtone-icon-card">
      <header class="dialtone-icon-card__header d-c-text h:d-bgc-unset">
        <div class="dialtone-icon-card__icon">
          <svg aria-hidden="true" focusable="false" data-name="Chevron Down" class="d-icon d-icon--chevronDown" viewBox="0 0 12 12"><path fill="currentColor" fill-rule="evenodd" d="M2.646 4.146a.5.5 0 0 1 .708 0L6 6.793l2.646-2.647a.5.5 0 1 1 .708.708l-3 3a.5.5 0 0 1-.708 0l-3-3a.5.5 0 0 1 0-.708Z" clip-rule="evenodd"/></svg>
        </div>
        <p class="dialtone-icon-card__subtitle d-tt-capitalize">
          Chevron Down
        </p>
      </header>
    </aside>
  </div>
  <div class="dialtone-icon-grid__item">
    <aside class="dialtone-icon-card">
      <header class="dialtone-icon-card__header d-c-text h:d-bgc-unset">
        <div class="dialtone-icon-card__icon">
          <svg aria-hidden="true" focusable="false" data-name="Chevron Left" class="d-icon d-icon--chevronLeft" viewBox="0 0 12 12"><path fill="currentColor" fill-rule="evenodd" d="M7.854 2.646a.5.5 0 0 1 0 .708L5.207 6l2.647 2.646a.5.5 0 1 1-.708.708l-3-3a.5.5 0 0 1 0-.708l3-3a.5.5 0 0 1 .708 0Z" clip-rule="evenodd"/></svg>
        </div>
        <p class="dialtone-icon-card__subtitle d-tt-capitalize">
          Chevron Left
        </p>
      </header>
    </aside>
  </div>
  <div class="dialtone-icon-grid__item">
    <aside class="dialtone-icon-card">
      <header class="dialtone-icon-card__header d-c-text h:d-bgc-unset">
        <div class="dialtone-icon-card__icon">
          <svg aria-hidden="true" focusable="false" data-name="Chevron Right" class="d-icon d-icon--chevronRight" viewBox="0 0 12 12"><path fill="currentColor" fill-rule="evenodd" d="M4.146 2.646a.5.5 0 0 1 .708 0l3 3a.5.5 0 0 1 0 .708l-3 3a.5.5 0 1 1-.708-.708L6.793 6 4.146 3.354a.5.5 0 0 1 0-.708Z" clip-rule="evenodd"/></svg>
        </div>
        <p class="dialtone-icon-card__subtitle d-tt-capitalize">
          Chevron Right
        </p>
      </header>
    </aside>
  </div>
  <div class="dialtone-icon-grid__item">
    <aside class="dialtone-icon-card">
      <header class="dialtone-icon-card__header d-c-text h:d-bgc-unset">
        <div class="dialtone-icon-card__icon">
          <svg aria-hidden="true" focusable="false" data-name="Chevron Up" class="d-icon d-icon--chevronUp" viewBox="0 0 12 12"><path fill="currentColor" fill-rule="evenodd" d="M5.646 4.146a.5.5 0 0 1 .708 0l3 3a.5.5 0 1 1-.708.708L6 5.207 3.354 7.854a.5.5 0 1 1-.708-.708l3-3Z" clip-rule="evenodd"/></svg>
        </div>
        <p class="dialtone-icon-card__subtitle d-tt-capitalize">
          Chevron Up
        </p>
      </header>
    </aside>
  </div>
  <div class="dialtone-icon-grid__item">
    <aside class="dialtone-icon-card">
      <header class="dialtone-icon-card__header d-c-text h:d-bgc-unset">
        <div class="dialtone-icon-card__icon">
          <svg aria-hidden="true" focusable="false" data-name="close" class="d-icon d-icon--close" viewBox="0 0 12 12"><path fill="currentColor" fill-rule="evenodd" d="M2.646 2.646a.5.5 0 0 1 .708 0L6 5.293l2.646-2.647a.5.5 0 1 1 .708.708L6.707 6l2.647 2.646a.5.5 0 1 1-.708.708L6 6.707 3.354 9.354a.5.5 0 1 1-.708-.708L5.293 6 2.646 3.354a.5.5 0 0 1 0-.708Z" clip-rule="evenodd"/></svg>
        </div>
        <p class="dialtone-icon-card__subtitle d-tt-capitalize">
          Close
        </p>
      </header>
    </aside>
  </div>
  <div class="dialtone-icon-grid__item">
    <aside class="dialtone-icon-card">
      <header class="dialtone-icon-card__header d-c-text h:d-bgc-unset">
        <div class="dialtone-icon-card__icon">
          <svg aria-hidden="true" focusable="false" data-name="Contacts" class="d-icon d-icon--contacts" width="13" height="12" viewBox="0 0 13 12"><path fill="currentColor" fill-rule="evenodd" d="M2.333.667a.5.5 0 0 1 .5-.5h7.334a.5.5 0 0 1 0 1H2.833a.5.5 0 0 1-.5-.5ZM1.48 1.98a.5.5 0 0 1 .353-.147h9.334a.5.5 0 0 1 .5.5v7.334a.5.5 0 0 1-.5.5H1.833a.5.5 0 0 1-.5-.5V2.333a.5.5 0 0 1 .147-.353Zm.853.853v6.334h1V9a1.833 1.833 0 0 1 1.834-1.833h2.666A1.833 1.833 0 0 1 9.667 9v.167h1V2.833H2.333Zm6.334 6.334V9a.833.833 0 0 0-.834-.833H5.167A.833.833 0 0 0 4.333 9v.167h4.334Zm-2.167-2a1.833 1.833 0 1 0 0-3.667 1.833 1.833 0 0 0 0 3.667Zm0-2.667a.833.833 0 1 0 0 1.667.833.833 0 0 0 0-1.667Zm-4.167 6.833a.5.5 0 0 1 .5-.5H10.3a.5.5 0 1 1 0 1H2.833a.5.5 0 0 1-.5-.5Z" clip-rule="evenodd"/></svg>
        </div>
        <p class="dialtone-icon-card__subtitle d-tt-capitalize">
          Contacts
        </p>
      </header>
    </aside>
  </div>
  <div class="dialtone-icon-grid__item">
    <aside class="dialtone-icon-card">
      <header class="dialtone-icon-card__header d-c-text h:d-bgc-unset">
        <div class="dialtone-icon-card__icon">
          <svg aria-hidden="true" focusable="false" data-name="Dialpad Ai" class="d-icon d-icon--dialpad ai" viewBox="0 0 12 12"><path fill="currentColor" fill-rule="evenodd" d="M10.25 2a.75.75 0 1 1 1.5 0 .75.75 0 0 1-1.5 0Zm-5.722.5c-.415 0-.886.352-1.108 1.058v.001L1.476 9.652a.5.5 0 1 1-.952-.304l1.943-6.09c0-.001 0 0 0 0C2.765 2.308 3.518 1.5 4.528 1.5s1.761.806 2.06 1.753l1.498 4.692C8.308 8.65 8.777 9 9.193 9c.735 0 1.307-.576 1.307-1.257V4.53a.5.5 0 0 1 1 0v3.212C11.5 9.003 10.454 10 9.193 10c-1.009 0-1.761-.806-2.06-1.753l-1.497-4.69v-.002C5.412 2.85 4.943 2.5 4.527 2.5Z" clip-rule="evenodd"/></svg>
        </div>
        <p class="dialtone-icon-card__subtitle d-tt-capitalize">
          Dialpad Ai
        </p>
      </header>
    </aside>
  </div>
  <div class="dialtone-icon-grid__item">
    <aside class="dialtone-icon-card">
      <header class="dialtone-icon-card__header d-c-text h:d-bgc-unset">
        <div class="dialtone-icon-card__icon">
          <svg aria-hidden="true" focusable="false" data-name="Hash" class="d-icon d-icon--hash" viewBox="0 0 12 12"><path fill="currentColor" fill-rule="evenodd" d="M5.055 1.003a.5.5 0 0 1 .442.552L5.225 4H7.22l.284-2.555a.5.5 0 1 1 .994.11L8.225 4H10a.5.5 0 0 1 0 1H8.114l-.222 2H10a.5.5 0 0 1 0 1H7.78l-.283 2.555a.5.5 0 0 1-.994-.11L6.775 8H4.78l-.284 2.555a.5.5 0 0 1-.994-.11L3.775 8H2a.5.5 0 1 1 0-1h1.886l.222-2H2a.5.5 0 1 1 0-1h2.22l.283-2.555a.5.5 0 0 1 .552-.442ZM5.115 5l-.223 2h1.994l.222-2H5.114Z" clip-rule="evenodd"/></svg>
        </div>
        <p class="dialtone-icon-card__subtitle d-tt-capitalize">
          Hash
        </p>
      </header>
    </aside>
  </div>
  <div class="dialtone-icon-grid__item">
    <aside class="dialtone-icon-card">
      <header class="dialtone-icon-card__header d-c-text h:d-bgc-unset">
        <div class="dialtone-icon-card__icon">
          <svg aria-hidden="true" focusable="false" data-name="Hash Bold" class="d-icon d-icon--hashBold" viewBox="0 0 12 12"><path fill="currentColor" fill-rule="evenodd" d="M4.068 1.397a.938.938 0 0 1 1.864.207l-.218 1.959h1.114l.24-2.166a.938.938 0 0 1 1.864.207l-.218 1.959H10a.937.937 0 1 1 0 1.875H8.506L8.38 6.563H10a.937.937 0 1 1 0 1.875H8.172l-.24 2.166a.937.937 0 0 1-1.864-.207l.218-1.96H5.172l-.24 2.167a.937.937 0 0 1-1.864-.207l.218-1.96H2a.937.937 0 1 1 0-1.874h1.494l.125-1.125H2a.937.937 0 1 1 0-1.875h1.828l.24-2.166ZM4.715 4.3a.501.501 0 0 1-.253.226.5.5 0 0 1 .156.174.501.501 0 0 1 .253-.226.5.5 0 0 1-.156-.174Zm2.747.226a.5.5 0 0 1 .156.174.501.501 0 0 1 .253-.226.5.5 0 0 1-.156-.174.501.501 0 0 1-.253.226ZM7.382 7.3a.501.501 0 0 1-.253.226.5.5 0 0 1 .156.174.501.501 0 0 1 .253-.226.5.5 0 0 1-.156-.174Zm-2.844.174a.5.5 0 0 1-.156-.174.501.501 0 0 1-.253.226.5.5 0 0 1 .156.174.501.501 0 0 1 .253-.226Zm.968-2.036L5.38 6.563h1.113l.125-1.125H5.506Z" clip-rule="evenodd"/><path fill="currentColor" fill-rule="evenodd" d="M5.048 1.065c.24.027.414.243.387.483l-.28 2.515h2.12l.29-2.611a.438.438 0 0 1 .87.096l-.28 2.515H10a.438.438 0 0 1 0 .875H8.058l-.236 2.125H10a.438.438 0 0 1 0 .875H7.725l-.29 2.61a.438.438 0 0 1-.87-.096l.28-2.514h-2.12l-.29 2.61a.438.438 0 0 1-.87-.096l.28-2.514H2a.438.438 0 0 1 0-.875h1.942l.236-2.125H2a.438.438 0 0 1 0-.875h2.275l.29-2.611a.438.438 0 0 1 .483-.387Zm.01 3.873-.236 2.125h2.12l.236-2.125h-2.12Z" clip-rule="evenodd"/></svg>
        </div>
        <p class="dialtone-icon-card__subtitle d-tt-capitalize">
          Hash Bold
        </p>
      </header>
    </aside>
  </div>
  <div class="dialtone-icon-grid__item">
    <aside class="dialtone-icon-card">
      <header class="dialtone-icon-card__header d-c-text h:d-bgc-unset">
        <div class="dialtone-icon-card__icon">
          <svg aria-hidden="true" focusable="false" data-name="Pause" class="d-icon d-icon--pause" viewBox="0 0 12 12"><path fill="currentColor" fill-rule="evenodd" d="M8 3a.5.5 0 0 1 .5.5v5a.5.5 0 1 1-1 0v-5A.5.5 0 0 1 8 3ZM4 3a.5.5 0 0 1 .5.5v5a.5.5 0 1 1-1 0v-5A.5.5 0 0 1 4 3Z" clip-rule="evenodd"/></svg>
        </div>
        <p class="dialtone-icon-card__subtitle d-tt-capitalize">
          Hold
        </p>
      </header>
    </aside>
  </div>
  <div class="dialtone-icon-grid__item">
    <aside class="dialtone-icon-card">
      <header class="dialtone-icon-card__header d-c-text h:d-bgc-unset">
        <div class="dialtone-icon-card__icon">
          <svg aria-hidden="true" focusable="false" data-name="Inbox" class="d-icon d-icon--inbox" viewBox="0 0 12 12"><path fill="currentColor" fill-rule="evenodd" d="M3.62 1.5h4.76a1.5 1.5 0 0 1 1.342.832l1.725 3.444A.5.5 0 0 1 11.5 6v3a1.5 1.5 0 0 1-1.5 1.5H2A1.5 1.5 0 0 1 .5 9V6a.5.5 0 0 1 .053-.224l1.724-3.443A1.503 1.503 0 0 1 3.62 1.5Zm0 1a.5.5 0 0 0-.447.277v.002L1.81 5.5H4a.5.5 0 0 1 .416.223L5.268 7h1.464l.852-1.277A.5.5 0 0 1 8 5.5h2.19L8.829 2.779v-.002A.5.5 0 0 0 8.38 2.5m2.12 4H8.268l-.852 1.277A.5.5 0 0 1 7 8H5a.5.5 0 0 1-.416-.223L3.732 6.5H1.5V9a.5.5 0 0 0 .5.5h8a.5.5 0 0 0 .5-.5V6.5Z" clip-rule="evenodd"/></svg>
        </div>
        <p class="dialtone-icon-card__subtitle d-tt-capitalize">
          Inbox
        </p>
      </header>
    </aside>
  </div>
  <div class="dialtone-icon-grid__item">
    <aside class="dialtone-icon-card">
      <header class="dialtone-icon-card__header d-c-text h:d-bgc-unset">
        <div class="dialtone-icon-card__icon">
          <svg aria-hidden="true" focusable="false" data-name="Info" class="d-icon d-icon--info" viewBox="0 0 12 12"><g clip-path="url(#a)"><path fill="currentColor" fill-rule="evenodd" d="M6 1.5a4.5 4.5 0 1 0 0 9 4.5 4.5 0 0 0 0-9ZM.5 6a5.5 5.5 0 1 1 11 0 5.5 5.5 0 0 1-11 0Zm5-2a.5.5 0 0 1 .5-.5h.005a.5.5 0 0 1 0 1H6a.5.5 0 0 1-.5-.5ZM6 5.5a.5.5 0 0 1 .5.5v2a.5.5 0 0 1-1 0V6a.5.5 0 0 1 .5-.5Z" clip-rule="evenodd"/></g><defs><clipPath id="a"><path fill="#fff" d="M0 0h12v12H0z"/></clipPath></defs></svg>
        </div>
        <p class="dialtone-icon-card__subtitle d-tt-capitalize">
          Info
        </p>
      </header>
    </aside>
  </div>
  <div class="dialtone-icon-grid__item">
    <aside class="dialtone-icon-card">
      <header class="dialtone-icon-card__header d-c-text h:d-bgc-unset">
        <div class="dialtone-icon-card__icon">
          <svg aria-hidden="true" focusable="false" data-name="Lock" class="d-icon d-icon--lock" viewBox="0 0 12 12"><g clip-path="url(#a)"><path fill="currentColor" fill-rule="evenodd" d="M2.5 6a.5.5 0 0 0-.5.5V10a.5.5 0 0 0 .5.5h7a.5.5 0 0 0 .5-.5V6.5a.5.5 0 0 0-.5-.5h-7ZM1 6.5A1.5 1.5 0 0 1 2.5 5h7A1.5 1.5 0 0 1 11 6.5V10a1.5 1.5 0 0 1-1.5 1.5h-7A1.5 1.5 0 0 1 1 10V6.5Z" clip-rule="evenodd"/><path fill="currentColor" fill-rule="evenodd" d="M6 1.5a2 2 0 0 0-2 2v2a.5.5 0 1 1-1 0v-2a3 3 0 1 1 6 0v2a.5.5 0 1 1-1 0v-2a2 2 0 0 0-2-2Z" clip-rule="evenodd"/></g><defs><clipPath id="a"><path fill="#fff" d="M0 0h12v12H0z"/></clipPath></defs></svg>
        </div>
        <p class="dialtone-icon-card__subtitle d-tt-capitalize">
          Lock
        </p>
      </header>
    </aside>
  </div>
  <div class="dialtone-icon-grid__item">
    <aside class="dialtone-icon-card">
      <header class="dialtone-icon-card__header d-c-text h:d-bgc-unset">
        <div class="dialtone-icon-card__icon">
          <svg aria-hidden="true" focusable="false" data-name="Lock Filled" class="d-icon d-icon--lockFilled" viewBox="0 0 12 12"><g clip-path="url(#a)"><path fill="currentColor" fill-rule="evenodd" d="M4.542 2.042A2.063 2.063 0 0 1 8.062 3.5v1.562H3.939V3.5c0-.547.217-1.072.604-1.458Zm-1.48 3.02V3.5a2.937 2.937 0 0 1 5.876 0v1.562H9.5c.794 0 1.438.644 1.438 1.438V10c0 .794-.644 1.438-1.438 1.438h-7A1.438 1.438 0 0 1 1.063 10V6.5c0-.794.643-1.437 1.437-1.437h.563Z" clip-rule="evenodd"/></g><defs><clipPath id="a"><path fill="#fff" d="M0 0h12v12H0z"/></clipPath></defs></svg>
        </div>
        <p class="dialtone-icon-card__subtitle d-tt-capitalize">
          Lock Filled
        </p>
      </header>
    </aside>
  </div>
  <div class="dialtone-icon-grid__item">
    <aside class="dialtone-icon-card">
      <header class="dialtone-icon-card__header d-c-text h:d-bgc-unset">
        <div class="dialtone-icon-card__icon">
          <svg aria-hidden="true" focusable="false" data-name="Unlock" class="d-icon d-icon--unlock" viewBox="0 0 12 12"><g clip-path="url(#a)"><path fill="currentColor" fill-rule="evenodd" d="M7.187 1.887A2 2 0 0 0 4 3.5V5h5.5A1.5 1.5 0 0 1 11 6.5V10a1.5 1.5 0 0 1-1.5 1.5h-7A1.5 1.5 0 0 1 1 10V6.5A1.5 1.5 0 0 1 2.5 5H3V3.5a3 3 0 0 1 5.94-.6.5.5 0 1 1-.98.2 2 2 0 0 0-.773-1.213ZM2 6.5a.5.5 0 0 1 .5-.5h7a.5.5 0 0 1 .5.5V10a.5.5 0 0 1-.5.5h-7A.5.5 0 0 1 2 10V6.5Z" clip-rule="evenodd"/></g><defs><clipPath id="a"><path fill="#fff" d="M0 0h12v12H0z"/></clipPath></defs></svg>
        </div>
        <p class="dialtone-icon-card__subtitle d-tt-capitalize">
          Unlock
        </p>
      </header>
    </aside>
  </div>
  <div class="dialtone-icon-grid__item">
    <aside class="dialtone-icon-card">
      <header class="dialtone-icon-card__header d-c-text h:d-bgc-unset">
        <div class="dialtone-icon-card__icon">
          <svg aria-hidden="true" focusable="false" data-name="Mic" class="d-icon d-icon--mic" viewBox="0 0 12 12"><path fill="currentColor" fill-rule="evenodd" d="M4.586 1.086A2 2 0 0 1 8 2.5V6a2 2 0 1 1-4 0V2.5a2 2 0 0 1 .586-1.414ZM6 1.5a1 1 0 0 0-1 1V6a1 1 0 0 0 2 0V2.5a1 1 0 0 0-1-1Zm-3.5 3A.5.5 0 0 1 3 5v1a3 3 0 1 0 6 0V5a.5.5 0 0 1 1 0v1a4 4 0 0 1-3.5 3.969V11H8a.5.5 0 0 1 0 1H4a.5.5 0 0 1 0-1h1.5V9.969A4 4 0 0 1 2 6V5a.5.5 0 0 1 .5-.5Z" clip-rule="evenodd"/></svg>
        </div>
        <p class="dialtone-icon-card__subtitle d-tt-capitalize">
          Mic
        </p>
      </header>
    </aside>
  </div>
  <div class="dialtone-icon-grid__item">
    <aside class="dialtone-icon-card">
      <header class="dialtone-icon-card__header d-c-text h:d-bgc-unset">
        <div class="dialtone-icon-card__icon">
          <svg aria-hidden="true" focusable="false" data-name="Mic Off" class="d-icon d-icon--micOff" viewBox="0 0 12 12"><g clip-path="url(#a)"><path fill="currentColor" fill-rule="evenodd" d="M.646.646a.5.5 0 0 1 .708 0l7.5 7.5 2.5 2.5a.5.5 0 0 1-.708.708L8.481 9.188a4 4 0 0 1-1.981.832V11H8a.5.5 0 0 1 0 1H4a.5.5 0 0 1 0-1h1.5v-.98A4.001 4.001 0 0 1 2 5.996V5a.5.5 0 0 1 1 0v1.007a3 3 0 0 0 4.768 2.468l-.753-.753A2 2 0 0 1 4 6V4.707L.646 1.354a.5.5 0 0 1 0-.708ZM5 5.707V6a1 1 0 0 0 1.258.965L5 5.707Zm.153-5.01A2 2 0 0 1 8 2.497V4.67a.5.5 0 0 1-1 0V2.502a1 1 0 0 0-1.893-.442.5.5 0 0 1-.894-.45 2 2 0 0 1 .94-.913ZM9.5 4.5a.5.5 0 0 1 .5.5v1.001c0 .235-.022.47-.063.702a.5.5 0 1 1-.984-.176A3.06 3.06 0 0 0 9 6V5a.5.5 0 0 1 .5-.5Z" clip-rule="evenodd"/></g><defs><clipPath id="a"><path fill="#fff" d="M0 0h12v12H0z"/></clipPath></defs></svg>
        </div>
        <p class="dialtone-icon-card__subtitle d-tt-capitalize">
          Mic Off
        </p>
      </header>
    </aside>
  </div>
  <div class="dialtone-icon-grid__item">
    <aside class="dialtone-icon-card">
      <header class="dialtone-icon-card__header d-c-text h:d-bgc-unset">
        <div class="dialtone-icon-card__icon">
          <svg aria-hidden="true" focusable="false" data-name="Message" class="d-icon d-icon--message" width="13" height="12" viewBox="0 0 13 12"><path fill="currentColor" fill-rule="evenodd" d="M3 2a.5.5 0 0 0-.5.5v6.793l1.146-1.147A.5.5 0 0 1 4 8h6a.5.5 0 0 0 .5-.5v-5A.5.5 0 0 0 10 2H3Zm-1.06-.56A1.5 1.5 0 0 1 3 1h7a1.5 1.5 0 0 1 1.5 1.5v5A1.5 1.5 0 0 1 10 9H4.207l-1.853 1.854A.5.5 0 0 1 1.5 10.5v-8c0-.398.158-.78.44-1.06Z" clip-rule="evenodd"/></svg>
        </div>
        <p class="dialtone-icon-card__subtitle d-tt-capitalize">
          Message
        </p>
      </header>
    </aside>
  </div>
  <div class="dialtone-icon-grid__item">
    <aside class="dialtone-icon-card">
      <header class="dialtone-icon-card__header d-c-text h:d-bgc-unset">
        <div class="dialtone-icon-card__icon">
          <svg aria-hidden="true" focusable="false" data-name="More Vertical" class="d-icon d-icon--moreVertical" viewBox="0 0 12 12"><path fill="currentColor" fill-rule="evenodd" d="M5 2.5a1 1 0 1 1 2 0 1 1 0 0 1-2 0ZM5 6a1 1 0 1 1 2 0 1 1 0 0 1-2 0Zm0 3.5a1 1 0 1 1 2 0 1 1 0 0 1-2 0Z" clip-rule="evenodd"/></svg>
        </div>
        <p class="dialtone-icon-card__subtitle d-tt-capitalize">
          More Vertical
        </p>
      </header>
    </aside>
  </div>
  <div class="dialtone-icon-grid__item">
    <aside class="dialtone-icon-card">
      <header class="dialtone-icon-card__header d-c-text h:d-bgc-unset">
        <div class="dialtone-icon-card__icon">
          <svg aria-hidden="true" focusable="false" data-name="More Horizontal" class="d-icon d-icon--moreHorizontal" viewBox="0 0 12 12"><path fill="currentColor" fill-rule="evenodd" d="M1.502 6a1 1 0 1 1 2 0 1 1 0 0 1-2 0Zm3.5 0a1 1 0 1 1 2 0 1 1 0 0 1-2 0Zm3.5 0a1 1 0 1 1 2 0 1 1 0 0 1-2 0Z" clip-rule="evenodd"/></svg>
        </div>
        <p class="dialtone-icon-card__subtitle d-tt-capitalize">
          More Horizontal
        </p>
      </header>
    </aside>
  </div>
  <div class="dialtone-icon-grid__item">
    <aside class="dialtone-icon-card">
      <header class="dialtone-icon-card__header d-c-text h:d-bgc-unset">
        <div class="dialtone-icon-card__icon">
          <svg aria-hidden="true" focusable="false" data-name="Phone" class="d-icon d-icon--phone" viewBox="0 0 12 12"><path fill="currentColor" fill-rule="evenodd" d="M2.01 2.034a.036.036 0 0 0-.003.016 7.98 7.98 0 0 0 .554 2.508l1.128-.94a.125.125 0 0 0 .041-.127l-.349-1.396A.125.125 0 0 0 3.26 2H2.083a.092.092 0 0 0-.065.023.035.035 0 0 0-.009.011ZM3 5.493l1.33-1.108c.332-.276.476-.718.371-1.136l-.349-1.397A1.125 1.125 0 0 0 3.26 1H2.083c-.577 0-1.106.457-1.074 1.1a9.101 9.101 0 0 0 2.674 5.94c2.217 2.22 4.922 2.773 6.137 2.912.67.077 1.18-.463 1.18-1.077V8.738c0-.516-.352-.966-.853-1.091l-1.439-.359a1.125 1.125 0 0 0-1.068.296L6.394 8.832A8.804 8.804 0 0 1 4.39 7.334 8.226 8.226 0 0 1 3 5.493Zm4.35 3.796a9.892 9.892 0 0 0 2.583.67.05.05 0 0 0 .023-.002.046.046 0 0 0 .017-.01.096.096 0 0 0 .026-.072V8.738a.125.125 0 0 0-.095-.121l-1.439-.359a.125.125 0 0 0-.118.033l-.997.998Z" clip-rule="evenodd"/></svg>
        </div>
        <p class="dialtone-icon-card__subtitle d-tt-capitalize">
          Phone
        </p>
      </header>
    </aside>
  </div>
  <div class="dialtone-icon-grid__item">
    <aside class="dialtone-icon-card">
      <header class="dialtone-icon-card__header d-c-text h:d-bgc-unset">
        <div class="dialtone-icon-card__icon">
          <svg aria-hidden="true" focusable="false" data-name="Phone Hang Up" class="d-icon d-icon--phoneHangUp" viewBox="0 0 12 12"><g clip-path="url(#a)"><path fill="currentColor" fill-rule="evenodd" d="M7.592 2.325a7.223 7.223 0 0 0-3.194 0l-.005.001a7.41 7.41 0 0 0-3.116 1.505.317.317 0 0 0-.11.23.331.331 0 0 0 .093.244l.002.002.863.877a.296.296 0 0 0 .397.035 5.652 5.652 0 0 1 1.093-.668h.001a.31.31 0 0 0 .131-.115.329.329 0 0 0 .051-.174v-.734a.5.5 0 0 1 .366-.481 6.838 6.838 0 0 1 3.685 0 .5.5 0 0 1 .365.481v.734c0 .062.019.122.051.174a.31.31 0 0 0 .131.115h.001c.389.18.755.404 1.094.668a.296.296 0 0 0 .394-.032l.003-.003.854-.868a.32.32 0 0 0 .092-.243.33.33 0 0 0-.11-.23 7.41 7.41 0 0 0-3.127-1.517h-.005Zm.22-.975a8.222 8.222 0 0 0-3.633 0A8.41 8.41 0 0 0 .64 3.06l-.007.006a1.317 1.317 0 0 0-.465.954 1.331 1.331 0 0 0 .381.988l.001.001.849.863a1.3 1.3 0 0 0 1.732.14l.003-.002a4.69 4.69 0 0 1 .902-.552 1.31 1.31 0 0 0 .554-.485 1.32 1.32 0 0 0 .208-.707V3.92a5.836 5.836 0 0 1 2.416 0v.346c.002.25.073.496.208.707.135.212.327.38.555.486l.21-.454-.21.453c.32.149.622.334.901.552l.004.002a1.296 1.296 0 0 0 1.72-.129l.853-.867a1.321 1.321 0 0 0 .378-.988 1.33 1.33 0 0 0-.465-.95l-.006-.005a8.411 8.411 0 0 0-3.55-1.723ZM6 6.236a.5.5 0 0 1 .5.5v2.41l.954-.955a.5.5 0 0 1 .707.707l-1.808 1.808a.5.5 0 0 1-.707 0L3.838 8.898a.5.5 0 1 1 .707-.707l.955.955v-2.41a.5.5 0 0 1 .5-.5Z" clip-rule="evenodd"/></g><defs><clipPath id="a"><path fill="#fff" d="M0 0h12v12H0z"/></clipPath></defs></svg>
        </div>
        <p class="dialtone-icon-card__subtitle d-tt-capitalize">
          Phone Hang Up
        </p>
      </header>
    </aside>
  </div>
  <div class="dialtone-icon-grid__item">
    <aside class="dialtone-icon-card">
      <header class="dialtone-icon-card__header d-c-text h:d-bgc-unset">
        <div class="dialtone-icon-card__icon">
          <svg aria-hidden="true" focusable="false" data-name="Search" class="d-icon d-icon--search" viewBox="0 0 12 12"><path fill="currentColor" fill-rule="evenodd" d="M5.5 2a3.5 3.5 0 1 0 0 7 3.5 3.5 0 0 0 0-7ZM1 5.5a4.5 4.5 0 1 1 9 0 4.5 4.5 0 0 1-9 0Z" clip-rule="evenodd"/><path fill="currentColor" fill-rule="evenodd" d="M7.972 7.972a.5.5 0 0 1 .707 0l2.175 2.174a.5.5 0 1 1-.707.708L7.972 8.679a.5.5 0 0 1 0-.707Z" clip-rule="evenodd"/></svg>
        </div>
        <p class="dialtone-icon-card__subtitle d-tt-capitalize">
          Search
        </p>
      </header>
    </aside>
  </div>
  <div class="dialtone-icon-grid__item">
    <aside class="dialtone-icon-card">
      <header class="dialtone-icon-card__header d-c-text h:d-bgc-unset">
        <div class="dialtone-icon-card__icon">
          <svg aria-hidden="true" focusable="false" data-name="Record" class="d-icon d-icon--record" viewBox="0 0 12 12"><path fill="currentColor" fill-rule="evenodd" d="M6 2a4 4 0 1 0 0 8 4 4 0 0 0 0-8ZM1 6a5 5 0 1 1 10 0A5 5 0 0 1 1 6Z" clip-rule="evenodd"/><path fill="currentColor" fill-rule="evenodd" d="M6 4a2 2 0 1 0 0 4 2 2 0 0 0 0-4ZM3 6a3 3 0 1 1 6 0 3 3 0 0 1-6 0Z" clip-rule="evenodd"/></svg>
        </div>
        <p class="dialtone-icon-card__subtitle d-tt-capitalize">
          Record
        </p>
      </header>
    </aside>
  </div>
  <div class="dialtone-icon-grid__item">
    <aside class="dialtone-icon-card">
      <header class="dialtone-icon-card__header d-c-text h:d-bgc-unset">
        <div class="dialtone-icon-card__icon">
          <svg aria-hidden="true" focusable="false" data-name="Shield Check" class="d-icon d-icon--shieldCheck" viewBox="0 0 12 12"><path fill="currentColor" fill-rule="evenodd" d="M5.824.532a.5.5 0 0 1 .352 0l4 1.5a.5.5 0 0 1 .324.468V6c0 1.723-1.14 3.1-2.17 4.001a11.338 11.338 0 0 1-2.053 1.418l-.038.02-.01.006-.004.001S6.223 11.448 6 11l-.224.447h-.001l-.004-.002-.01-.006-.038-.02a10.362 10.362 0 0 1-.613-.355c-.39-.244-.914-.603-1.44-1.063C2.64 9.101 1.5 7.723 1.5 6V2.5a.5.5 0 0 1 .324-.468l4-1.5ZM6 11l-.224.447c.14.07.308.07.448 0L6 11Zm0-.569a10.341 10.341 0 0 0 1.67-1.183C8.64 8.4 9.5 7.278 9.5 6V2.846L6 1.534 2.5 2.846V6c0 1.277.86 2.4 1.83 3.249A10.341 10.341 0 0 0 6 10.43Zm1.854-5.785a.5.5 0 0 1 0 .708l-2 2a.5.5 0 0 1-.708 0l-1-1a.5.5 0 1 1 .708-.708l.646.647 1.646-1.647a.5.5 0 0 1 .708 0Z" clip-rule="evenodd"/></svg>
        </div>
        <p class="dialtone-icon-card__subtitle d-tt-capitalize">
          Shield Check
        </p>
      </header>
    </aside>
  </div>
  <div class="dialtone-icon-grid__item">
    <aside class="dialtone-icon-card">
      <header class="dialtone-icon-card__header d-c-text h:d-bgc-unset">
        <div class="dialtone-icon-card__icon">
          <svg aria-hidden="true" focusable="false" data-name="Satisfied" class="d-icon d-icon--satisfied" viewBox="0 0 12 12"><g clip-path="url(#a)"><path fill="currentColor" fill-rule="evenodd" d="M6 1.5a4.5 4.5 0 1 0 0 9 4.5 4.5 0 0 0 0-9ZM.5 6a5.5 5.5 0 1 1 11 0 5.5 5.5 0 0 1-11 0ZM4 4.5a.5.5 0 0 1 .5-.5h.005a.5.5 0 0 1 0 1H4.5a.5.5 0 0 1-.5-.5Zm3 0a.5.5 0 0 1 .5-.5h.005a.5.5 0 0 1 0 1H7.5a.5.5 0 0 1-.5-.5ZM3.7 6.6a.5.5 0 0 1 .698.098L4.4 6.7 4 7l.4-.3v-.001.002l.018.02a2.474 2.474 0 0 0 .408.374c.288.21.687.405 1.174.405.487 0 .886-.195 1.175-.404a2.474 2.474 0 0 0 .424-.395l.003-.003A.5.5 0 0 1 8.4 7.3L8 7l.4.3-.001.001-.001.002-.004.005-.01.014c-.01.01-.021.025-.036.042a3.474 3.474 0 0 1-.585.54c-.4.29-1 .596-1.763.596a2.994 2.994 0 0 1-1.763-.596 3.474 3.474 0 0 1-.62-.582l-.01-.014-.005-.005v-.002H3.6L4 7l-.4.3a.5.5 0 0 1 .1-.7Z" clip-rule="evenodd"/></g><defs><clipPath id="a"><path fill="#fff" d="M0 0h12v12H0z"/></clipPath></defs></svg>
        </div>
        <p class="dialtone-icon-card__subtitle d-tt-capitalize">
          Satisfied
        </p>
      </header>
    </aside>
  </div>
  <div class="dialtone-icon-grid__item">
    <aside class="dialtone-icon-card">
      <header class="dialtone-icon-card__header d-c-text h:d-bgc-unset">
        <div class="dialtone-icon-card__icon">
          <svg aria-hidden="true" focusable="false" data-name="Settings" class="d-icon d-icon--settings" viewBox="0 0 12 12"><g clip-path="url(#a)"><path fill="currentColor" fill-rule="evenodd" d="M6 4a2 2 0 1 0 0 4 2 2 0 0 0 0-4ZM5 6a1 1 0 1 1 2 0 1 1 0 0 1-2 0Z" clip-rule="evenodd"/><path fill="currentColor" fill-rule="evenodd" d="M4.83.94A1.5 1.5 0 0 1 5.89.5h.22A1.5 1.5 0 0 1 7.61 2v.093a.5.5 0 0 0 .25.429h.001l.215.125a.5.5 0 0 0 .522-.012l.06-.033a1.5 1.5 0 0 1 2.04.552l.11.19a1.502 1.502 0 0 1-.544 2.047l-.072.043-.011.006a.5.5 0 0 0-.246.433v.254a.5.5 0 0 0 .244.431l.066.036.016.008a1.5 1.5 0 0 1 .548 2.046l-.111.198a1.5 1.5 0 0 1-2.04.552l-.064-.035a.5.5 0 0 0-.518-.01l-.216.125a.5.5 0 0 0-.25.429V10a1.5 1.5 0 0 1-1.5 1.5h-.22a1.5 1.5 0 0 1-1.5-1.5v-.09a.5.5 0 0 0-.25-.432h-.001l-.215-.125a.5.5 0 0 0-.52.011l-.062.034a1.5 1.5 0 0 1-2.04-.552l-.11-.19a1.502 1.502 0 0 1 .544-2.047l.072-.043.011-.007a.5.5 0 0 0 .246-.432v-.255a.5.5 0 0 0-.25-.429.506.506 0 0 1-.034-.021l-.057-.038a1.5 1.5 0 0 1-.532-2.039l.11-.19a1.5 1.5 0 0 1 2.04-.553l.068.037a.5.5 0 0 0 .514.009l.216-.126a.5.5 0 0 0 .25-.433V2c0-.398.158-.78.44-1.06Zm4.105 4.934v.252a1.5 1.5 0 0 0 .77 1.313l.06.032a.5.5 0 0 1 .177.68l-.11.195a.5.5 0 0 1-.682.18l-.015-.007-.068-.037a1.5 1.5 0 0 0-1.492.005l-.216.125A1.5 1.5 0 0 0 6.61 9.91V10a.5.5 0 0 1-.5.5h-.22a.5.5 0 0 1-.5-.5v-.09a1.5 1.5 0 0 0-.75-1.298l-.214-.124-.001-.001a1.5 1.5 0 0 0-1.49-.005l-.07.037-.015.008a.5.5 0 0 1-.682-.183l-.11-.19a.5.5 0 0 1 .183-.681l.075-.046a1.5 1.5 0 0 0 .749-1.303v-.257a1.5 1.5 0 0 0-.734-1.28l-.064-.043a.5.5 0 0 1-.21-.7l.11-.189a.5.5 0 0 1 .683-.182l.015.008.067.036a1.5 1.5 0 0 0 1.493-.004l.216-.125a1.5 1.5 0 0 0 .749-1.295V2a.5.5 0 0 1 .5-.5h.22a.5.5 0 0 1 .5.5v.09a1.5 1.5 0 0 0 .75 1.298l.214.124.001.001a1.5 1.5 0 0 0 1.49.006l.07-.038.015-.008a.5.5 0 0 1 .682.183l.11.19a.5.5 0 0 1-.183.681l-.076.046a1.5 1.5 0 0 0-.548.549v.002a1.5 1.5 0 0 0-.2.75Z" clip-rule="evenodd"/></g><defs><clipPath id="a"><path fill="#fff" d="M0 0h12v12H0z"/></clipPath></defs></svg>
        </div>
        <p class="dialtone-icon-card__subtitle d-tt-capitalize">
          Settings
        </p>
      </header>
    </aside>
  </div>
  <div class="dialtone-icon-grid__item">
    <aside class="dialtone-icon-card">
      <header class="dialtone-icon-card__header d-c-text h:d-bgc-unset">
        <div class="dialtone-icon-card__icon">
          <svg aria-hidden="true" focusable="false" data-name="Share Screen" class="d-icon d-icon--shareScreen" viewBox="0 0 12 12"><path fill="currentColor" fill-rule="evenodd" d="M.5 2.5A1.5 1.5 0 0 1 2 1h8a1.5 1.5 0 0 1 1.5 1.5v5A1.5 1.5 0 0 1 10 9H2A1.5 1.5 0 0 1 .5 7.5v-5ZM2 2a.5.5 0 0 0-.5.5v5A.5.5 0 0 0 2 8h8a.5.5 0 0 0 .5-.5v-5A.5.5 0 0 0 10 2H2Zm4.98.813a.5.5 0 0 1 .707 0l1.111 1.111a.5.5 0 0 1 0 .707L7.687 5.742a.5.5 0 0 1-.707-.707l.257-.257H5.111a.389.389 0 0 0-.389.389v1.555a.5.5 0 0 1-1 0V5.167a1.389 1.389 0 0 1 1.39-1.389h2.125L6.98 3.52a.5.5 0 0 1 0-.707ZM.5 10.5A.5.5 0 0 1 1 10h10a.5.5 0 0 1 0 1H1a.5.5 0 0 1-.5-.5Z" clip-rule="evenodd"/></svg>
        </div>
        <p class="dialtone-icon-card__subtitle d-tt-capitalize">
          Share Screen
        </p>
      </header>
    </aside>
  </div>
  <div class="dialtone-icon-grid__item">
    <aside class="dialtone-icon-card">
      <header class="dialtone-icon-card__header d-c-text h:d-bgc-unset">
        <div class="dialtone-icon-card__icon">
          <svg aria-hidden="true" focusable="false" data-name="Star" class="d-icon d-icon--star" viewBox="0 0 12 12"><g clip-path="url(#a)"><path fill="currentColor" fill-rule="evenodd" d="M6 .5a.5.5 0 0 1 .448.279l1.43 2.894 3.194.467a.5.5 0 0 1 .277.853L9.037 7.245l.546 3.18a.5.5 0 0 1-.726.527L6 9.45l-2.857 1.503a.5.5 0 0 1-.726-.527l.546-3.181L.65 4.993a.5.5 0 0 1 .277-.853l3.195-.467L5.552.78A.5.5 0 0 1 6 .5Zm0 1.63L4.903 4.35a.5.5 0 0 1-.376.274l-2.453.358 1.776 1.73a.5.5 0 0 1 .144.443l-.419 2.44 2.193-1.153a.5.5 0 0 1 .466 0l2.193 1.154-.419-2.441a.5.5 0 0 1 .144-.443l1.775-1.729-2.453-.358a.5.5 0 0 1-.376-.274L6 2.13Z" clip-rule="evenodd"/></g><defs><clipPath id="a"><path fill="#fff" d="M0 0h12v12H0z"/></clipPath></defs></svg>
        </div>
        <p class="dialtone-icon-card__subtitle d-tt-capitalize">
          Star
        </p>
      </header>
    </aside>
  </div>
  <div class="dialtone-icon-grid__item">
    <aside class="dialtone-icon-card">
      <header class="dialtone-icon-card__header d-c-text h:d-bgc-unset">
        <div class="dialtone-icon-card__icon">
          <svg aria-hidden="true" focusable="false" data-name="Star Filled" class="d-icon d-icon--starFilled" viewBox="0 0 12 12"><g clip-path="url(#a)"><path fill="currentColor" d="m6 1 1.545 3.13L11 4.635 8.5 7.07l.59 3.44L6 8.885 2.91 10.51l.59-3.44L1 4.635l3.455-.505L6 1Z"/><path fill="currentColor" fill-rule="evenodd" d="M6 .5a.5.5 0 0 1 .448.279l1.43 2.894 3.194.467a.5.5 0 0 1 .277.853L9.037 7.245l.546 3.18a.5.5 0 0 1-.726.527L6 9.45l-2.857 1.503a.5.5 0 0 1-.726-.527l.546-3.181L.65 4.993a.5.5 0 0 1 .277-.853l3.195-.467L5.552.78A.5.5 0 0 1 6 .5Zm0 1.63L4.903 4.35a.5.5 0 0 1-.376.274l-2.453.358 1.776 1.73a.5.5 0 0 1 .144.443l-.419 2.44 2.193-1.153a.5.5 0 0 1 .466 0l2.193 1.154-.419-2.441a.5.5 0 0 1 .144-.443l1.775-1.729-2.453-.358a.5.5 0 0 1-.376-.274L6 2.13Z" clip-rule="evenodd"/></g><defs><clipPath id="a"><path fill="#fff" d="M0 0h12v12H0z"/></clipPath></defs></svg>
        </div>
        <p class="dialtone-icon-card__subtitle d-tt-capitalize">
          Star Filled
        </p>
      </header>
    </aside>
  </div>
  <div class="dialtone-icon-grid__item">
    <aside class="dialtone-icon-card">
      <header class="dialtone-icon-card__header d-c-text h:d-bgc-unset">
        <div class="dialtone-icon-card__icon">
          <svg aria-hidden="true" focusable="false" data-name="User" class="d-icon d-icon--user" viewBox="0 0 12 12"><path fill="currentColor" fill-rule="evenodd" d="M6 2a1.5 1.5 0 1 0 0 3 1.5 1.5 0 0 0 0-3ZM3.5 3.5a2.5 2.5 0 1 1 5 0 2.5 2.5 0 0 1-5 0Zm-.768 4.232A2.5 2.5 0 0 1 4.5 7h3A2.5 2.5 0 0 1 10 9.5v1a.5.5 0 0 1-1 0v-1A1.5 1.5 0 0 0 7.5 8h-3A1.5 1.5 0 0 0 3 9.5v1a.5.5 0 0 1-1 0v-1a2.5 2.5 0 0 1 .732-1.768Z" clip-rule="evenodd"/></svg>
        </div>
        <p class="dialtone-icon-card__subtitle d-tt-capitalize">
          User
        </p>
      </header>
    </aside>
  </div>
  <div class="dialtone-icon-grid__item">
    <aside class="dialtone-icon-card">
      <header class="dialtone-icon-card__header d-c-text h:d-bgc-unset">
        <div class="dialtone-icon-card__icon">
          <svg aria-hidden="true" focusable="false" data-name="Users" class="d-icon d-icon--users" viewBox="0 0 12 12"><path fill="currentColor" fill-rule="evenodd" d="M4.5 2a1.5 1.5 0 1 0 0 3 1.5 1.5 0 0 0 0-3ZM2 3.5a2.5 2.5 0 1 1 5 0 2.5 2.5 0 0 1-5 0Zm5.516-2.059a.5.5 0 0 1 .608-.36 2.5 2.5 0 0 1 0 4.843.5.5 0 0 1-.248-.968 1.5 1.5 0 0 0 0-2.907.5.5 0 0 1-.36-.608ZM1.232 7.732A2.5 2.5 0 0 1 3 7h3a2.5 2.5 0 0 1 2.5 2.5v1a.5.5 0 0 1-1 0v-1A1.5 1.5 0 0 0 6 8H3a1.5 1.5 0 0 0-1.5 1.5v1a.5.5 0 0 1-1 0v-1a2.5 2.5 0 0 1 .732-1.768Zm7.784-.292a.5.5 0 0 1 .609-.36A2.5 2.5 0 0 1 11.5 9.5v1a.5.5 0 0 1-1 0v-1a1.5 1.5 0 0 0-1.125-1.45.5.5 0 0 1-.36-.61Z" clip-rule="evenodd"/></svg>
        </div>
        <p class="dialtone-icon-card__subtitle d-tt-capitalize">
          Users
        </p>
      </header>
    </aside>
  </div>
  <div class="dialtone-icon-grid__item">
    <aside class="dialtone-icon-card">
      <header class="dialtone-icon-card__header d-c-text h:d-bgc-unset">
        <div class="dialtone-icon-card__icon">
          <svg aria-hidden="true" focusable="false" data-name="Video" class="d-icon d-icon--video" viewBox="0 0 12 12"><path fill="currentColor" fill-rule="evenodd" d="M2 3.5a.5.5 0 0 0-.5.5v4a.5.5 0 0 0 .5.5h5a.5.5 0 0 0 .5-.5V4a.5.5 0 0 0-.5-.5H2Zm6.5 1.566V4A1.5 1.5 0 0 0 7 2.5H2A1.5 1.5 0 0 0 .5 4v4A1.5 1.5 0 0 0 2 9.5h5A1.5 1.5 0 0 0 8.5 8V6.934l2.223 1.482A.5.5 0 0 0 11.5 8V4a.5.5 0 0 0-.777-.416L8.5 5.066ZM8.901 6 10.5 7.066V4.934L8.901 6Z" clip-rule="evenodd"/></svg>
        </div>
        <p class="dialtone-icon-card__subtitle d-tt-capitalize">
          Video
        </p>
      </header>
    </aside>
  </div>
  <div class="dialtone-icon-grid__item">
    <aside class="dialtone-icon-card">
      <header class="dialtone-icon-card__header d-c-text h:d-bgc-unset">
        <div class="dialtone-icon-card__icon">
          <svg aria-hidden="true" focusable="false" data-name="Video Off" class="d-icon d-icon--videoOff" viewBox="0 0 12 12"><g clip-path="url(#a)"><path fill="currentColor" fill-rule="evenodd" d="M.646.646a.5.5 0 0 1 .708 0l10 10a.5.5 0 0 1-.708.708l-2.45-2.45A1.5 1.5 0 0 1 7 9.5H2A1.5 1.5 0 0 1 .5 8V4a1.5 1.5 0 0 1 1.306-1.487l-1.16-1.16a.5.5 0 0 1 0-.707ZM2.793 3.5H2a.5.5 0 0 0-.5.5v4a.5.5 0 0 0 .5.5h5a.5.5 0 0 0 .468-.325L2.793 3.5ZM7 3.5H5.33a.5.5 0 0 1 0-1H7A1.5 1.5 0 0 1 8.5 4v.963l.063.063 2.16-1.442A.5.5 0 0 1 11.5 4v4a.5.5 0 0 1-1 0V4.935l-1.722 1.15a.5.5 0 0 1-.632-.061l-.5-.5A.5.5 0 0 1 7.5 5.17V4a.5.5 0 0 0-.5-.5Z" clip-rule="evenodd"/></g><defs><clipPath id="a"><path fill="#fff" d="M0 0h12v12H0z"/></clipPath></defs></svg>
        </div>
        <p class="dialtone-icon-card__subtitle d-tt-capitalize">
          Video Off
        </p>
      </header>
    </aside>
  </div>
</div>

## Usage

SVG icons are currently available from `@dialpad/dialtone/lib/dist/svg/v7/icon-name.svg`

## Variants and examples

### Base Styles

<code-well-header>
    <inbox-icon></inbox-icon>
</code-well-header>

```html
<svg
  aria-hidden="true"
  focusable="false"
  data-name="[ICON_NAME]"
  class="
    d-icon
    d-icon--size-[SIZE]
    d-icon--[ICON_NAME]
  "
  viewBox="0 0 12 12"
>
  ...
</svg>
```

## Sizes

<table class="d-table dialtone-doc-table">
  <thead>
    <tr>
      <th scope="col">Size</th>
      <th scope="col">Width and height</th>
      <th scope="col" class="d-w35p">Class</th>
      <th scope="col" class="d-ta-center">Example</th>
      <th scope="col"><span class="d-vi-visible-sr">Exception</span></th>
    </tr>
  </thead>
  <tbody>
    <tr v-for="{size, width_height, className, deviceOnly } in sizes">
      <th scope="row">{{ size }}</th>
      <td class="d-ff-mono d-fs-100">{{ width_height }}</td>
      <td class="d-ff-mono d-fs-100">.{{ className }}</td>
      <td class="d-ta-center">
        <inbox-icon :class="className" />
      </td>
      <td class="d-ta-right">
        <span v-if="deviceOnly" class="d-badge d-mr8 d-mb8 d-badge">Device only</span>
      </td>
    </tr>
  </tbody>
</table>

<script setup>
    import InboxIcon from '@v7Icons/Inbox.vue';
    import { sizes } from '@data/icon.json';
</script>
