    ...
    <table class="d-table dialtone-doc-table">
      <thead>
        <tr>
          ...
          <th class="d-w30p" scope="col">Pixel Equivalent</th>
          <th scope="col">Actual output</th>
        </tr>
      </thead>
      <tbody>
        <tr v-for="{value: val, output} in values">
          <th class="d-ff-mono d-fc-purple d-fw-normal d-fs12" scope="row">
            <span v-if="kind === 'css'">var(--su{{ val }})</span>
            <span v-else>@su{{ val }}</span>
          </th>
          <td class="d-ff-mono d-fc-orange d-fs12">{{ val }}px</td>
          <td class="d-ff-mono d-fc-orange d-fs12">{{ output }}</td>
        </tr>
      </tbody>
      <tbody v-if="kind === 'css'">
        <tr v-for="{value: val, output} in negativeValues">
          <th class="d-ff-mono d-fc-purple d-fw-normal d-fs12" scope="row">
            var(--sun{{ val }})
          </th>
          <td class="d-ff-mono d-fc-orange d-fs12">-{{ val }}px</td>
